---
title: "OTA Updates: Wireless Firmware Flashing in ESPHome"
sidebarTitle: "OTA"
...
ESPHome supports updating device firmware wirelessly without a physical USB connection. As of ESPHome 2024.6.0, OTA functionality is organized as a *platform component* — each update mechanism is a separate platform under the `ota:` list key. The most common platform is `esphome`, which implements the native OTA protocol used by the ESPHome dashboard and CLI. Additional platforms include `http_request` for pull-based URL updates, `web_server` for browser-based uploads, and `zephyr_mcumgr` for nRF52 boards. Safe mode (automatic recovery) is automatically enabled alongside OTA and can be found at the [Safe Mode](/guides/ota-updates#safe-mode) section below.
...
For full safe mode configuration options, see the [Safe Mode Guide](/guides/ota-updates).
